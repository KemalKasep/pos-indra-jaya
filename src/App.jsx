...
const App = () => {
  ...
  const ubahQty = (kode, delta) => setKeranjang(prev => prev.map(k => k.kode === kode ? { ...k, qty: Math.max(1, (parseInt(k.qty)||0) + delta) } : k));
  ...
  const prosesInputSaldo = async () => {
    const nominal = prompt("Masukkan jumlah Saldo Awal (CASH):\nContoh: 150000");
    if (!nominal) return;
    ...
    try {
      ...
      const result = await response.json();
      if (result.status === "success") { alert("Saldo tersimpan!"); setRingkasan(prev => ({ ...prev, saldoAwal: angka })); }
    } catch (e) { alert("Error jaringan."); } finally { setIsProcessing(false); }
  };
      ...
      <div style={{ display: 'flex', height: '100vh', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f1f5f9', padding: '20px' }}>
        <div style={{ backgroundColor: 'white', padding: isMobile ? '30px 20px' : '40px', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', width: '100%', maxWidth: '350px' }}>
           ...
           <form onSubmit={handleLogin}>
              <div style={{ marginBottom: '15px' }}><label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: 'bold', color: '#1e293b' }}>Username</label><input type="text" value={username} onChange={e=>setUsername(e.target.value)} placeholder="Masukkan username" style={{ width: '100%', boxSizing: 'border-box', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', color: '#1e293b', backgroundColor: 'white' }} required /></div>
              <div style={{ marginBottom: '25px' }}><label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: 'bold', color: '#1e293b' }}>PIN</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Masukkan PIN" style={{ width: '100%', boxSizing: 'border-box', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', color: '#1e293b', backgroundColor: 'white' }} required /></div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', height: '100vh', fontFamily: "'Segoe UI', Roboto, sans-serif", backgroundColor: '#f3f4f6' }}>
      ...
      {/* SIDEBAR (HANYA MUNCUL DI LAPTOP) */}
      {!isMobile && (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingBottom: isMobile ? '60px' : '0' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden', padding: isMobile ? '10px' : '20px 30px', backgroundImage: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)' }}>
          ...
          {activeTab === 'KASIR' && role === 'ADMIN' && (
            <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', height: '100%', gap: isMobile ? '15px' : '25px' }}>
              
              {/* PRODUK KIRI (ATAS JIKA DI HP) */}
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ position: 'relative', marginBottom: isMobile ? '10px' : '20px', flexShrink: 0 }}>
                  <span style={{ position: 'absolute', left: '15px', top: '15px', fontSize: '18px' }}>🔍</span>
                  <input ref={scannerRef} type="text" placeholder="Cari nama / scan..." value={keyword} onChange={e => setKeyword(e.target.value)} onKeyDown={handleScanner} disabled={isProcessing} style={{ width: '100%', boxSizing: 'border-box', padding: '15px 15px 15px 45px', fontSize: '16px', borderRadius: '10px', border: '1px solid #e2e8f0', outline: 'none', color: '#1e293b', backgroundColor: 'white' }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(220px, 1fr))', gap: '15px', overflowY: 'auto', paddingBottom: '10px' }}>
              ...
              {/* KERANJANG KANAN (BAWAH JIKA DI HP) */}
              <div style={{ height: isMobile ? '45%' : '100%', width: isMobile ? '100%' : '360px', backgroundColor: 'white', borderRadius: '16px', display: 'flex', flexDirection: 'column', flexShrink: 0, border: '1px solid #e2e8f0' }}>
                <div style={{ padding: '15px 20px', borderBottom: '1px solid #f1f5f9' }}><h3 style={{ margin: 0, fontSize: '16px', color: '#1e293b' }}>🛒 Keranjang</h3></div>
                <div style={{ flex: 1, overflowY: 'auto', padding: '10px 15px' }}>
                  {keranjang.map(k => (
                    <div key={k.kode} style={{ display: 'flex', gap: '10px', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '10px' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}><div style={{ fontWeight: '600', fontSize: '13px', color: '#1e293b' }}>{k.nama}</div><button onClick={() => hapusItem(k.kode)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: '16px' }}>🗑️</button></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e2e8f0', borderRadius: '6px', overflow: 'hidden' }}>
                            <button onClick={() => ubahQty(k.kode, -1)} style={{ padding: '4px 12px', border: 'none', background: '#f1f5f9', color: '#1e293b', cursor: 'pointer', fontWeight: 'bold' }}>-</button>
                            <input type="number" value={k.qty} onChange={(e) => ubahQtyKetikan(k.kode, e.target.value)} onBlur={(e) => validasiQty(k.kode, e.target.value)} style={{ width: '35px', textAlign: 'center', border: 'none', outline: 'none', fontWeight: 'bold', fontSize: '13px', backgroundColor: 'white', color: '#1e293b', padding: '5px 0' }} />
                            <button onClick={() => ubahQty(k.kode, 1)} style={{ padding: '4px 12px', border: 'none', background: '#f1f5f9', color: '#1e293b', cursor: 'pointer', fontWeight: 'bold' }}>+</button>
                          </div>
                ...
                <div style={{ padding: '15px 20px', borderTop: '2px dashed #e2e8f0' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}><span style={{ fontSize: '16px', fontWeight: 'bold', color: '#1e293b' }}>TOTAL</span><span style={{ fontSize: '20px', fontWeight: 'bold', color: '#10b981' }}>Rp {totalAkhir.toLocaleString('id-ID')}</span></div>
                  ...
                  <button onClick={prosesCheckout} disabled={isProcessing || keranjang.length === 0} style={{ width: '100%', padding: '12px', backgroundColor: isProcessing || keranjang.length === 0 ? '#94a3b8' : '#8b5cf6', color: 'white', fontWeight: 'bold', border: 'none', borderRadius: '8px' }}>{isProcessing ? 'MEMPROSES...' : 'BAYAR SEKARANG'}</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ height: '100%', overflowY: 'auto' }}>
              ...
              <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0', overflowX: 'auto' }}>
                ...
                <table style={{ width: '100%', minWidth: '500px', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                  <thead><tr style={{ borderBottom: '2px solid #e2e8f0', color: '#64748b' }}><th style={{ padding: '10px' }}>Struk</th><th style={{ padding: '10px' }}>Barang</th><th style={{ padding: '10px' }}>Qty</th><th style={{ padding: '10px' }}>Total</th><th style={{ padding: '10px' }}>Tipe</th></tr></thead>
                  <tbody>{riwayat.map((r, i) => (<tr key={i} style={{ borderBottom: '1px solid #f1f5f9', color: '#1e293b' }}><td style={{ padding: '10px' }}>{r.noStruk}</td><td style={{ padding: '10px' }}>{r.nama}</td><td style={{ padding: '10px' }}>{r.qty}</td><td style={{ padding: '10px', color: '#10b981', fontWeight: 'bold' }}>Rp {(r.total || 0).toLocaleString('id-ID')}</td><td style={{ padding: '10px' }}>{r.pembayaran}</td></tr>))}</tbody>
                </table>
              </div>
            </div>
          )}
      ...
      {/* MENU NAVIGASI BAWAH (HANYA MUNCUL DI HP) */}
      {isMobile && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: 'white', display: 'flex', justifyContent: 'space-around', padding: '10px 5px', zIndex: 100, borderTop: '1px solid #e2e8f0', boxShadow: '0 -2px 10px rgba(0,0,0,0.05)' }}>
          {role === 'ADMIN' ? (
            <>
              <button onClick={() => setActiveTab('KASIR')} style={{ background: 'none', border: 'none', color: activeTab === 'KASIR' ? '#3b82f6' : '#94a3b8', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 'bold' }}><span style={{ fontSize: '20px' }}>🛒</span>Kasir</button>
              <button onClick={() => setActiveTab('RIWAYAT')} style={{ background: 'none', border: 'none', color: activeTab === 'RIWAYAT' ? '#3b82f6' : '#94a3b8', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 'bold' }}><span style={{ fontSize: '20px' }}>⏱️</span>Riwayat</button>
              <button onClick={() => setActiveTab('UTILITY')} style={{ background: 'none', border: 'none', color: activeTab === 'UTILITY' ? '#3b82f6' : '#94a3b8', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 'bold' }}><span style={{ fontSize: '20px' }}>🛠️</span>Utility</button>
            </>
          ) : (
